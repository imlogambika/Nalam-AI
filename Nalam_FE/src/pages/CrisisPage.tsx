...
import { getCrisisContacts, type CrisisContact } from "@/services/api";
...
const CrisisPage = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const response = await getCrisisContacts();
        if (response.contacts && response.contacts.length > 0) {
          setContacts(
            response.contacts.map((c: CrisisContact) => ({
              ...c,
              desc: "Crisis helpline",
            }))
          );
        }
      } catch (e) {
        console.log("Using fallback crisis contacts");
      }
    };
    fetchContacts();
  }, []);
